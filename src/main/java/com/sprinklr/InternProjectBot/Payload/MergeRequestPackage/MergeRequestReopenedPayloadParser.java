...
public class MergeRequestReopenedPayloadParser extends MergeRequestPayloadParser{

    public MergeRequestReopenedPayloadParser(GitLabAPI gitLabAPI){
        super(gitLabAPI);
    }
    ...
    public List<Message> getMessagesToBeSent() {
        ...
        messageBody.setMessageTypeID(MessageTypeID.MERGE_REQUEST_REOPENED);
    ...
    private MessageBody constructMessageBody(){
        ...
        String messageToSend = "Hello! , [" + this.authorName + "]("
                +  this.authorURL + ")"
                + " has reopened the merge request: [" +this.mergeRequestTitle
                + "](" + this.viewLink + ") "
    ...
    private MessageBody constructMessageBodyInHtml(){
        String titile = "Merge request has Reopened";
        String messageToSend = "Hello! , " + HtmlContentCreator.getAnchor(this.authorURL, this.authorName)
                + " has reopened the merge request: " + HtmlContentCreator.getAnchor(this.viewLink, this.mergeRequestTitle)
