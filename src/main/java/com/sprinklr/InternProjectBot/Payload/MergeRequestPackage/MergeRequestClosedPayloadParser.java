...
public class MergeRequestClosedPayloadParser extends MergeRequestPayloadParser{

    public MergeRequestClosedPayloadParser(GitLabAPI gitLabAPI){
        super(gitLabAPI);
    }
    ...
    public List<Message> getMessagesToBeSent() {
        ...
        messageBody.setMessageTypeID(MessageTypeID.MERGE_REQUEST_CLOSED);
    ...
    private MessageBody constructMessageBody(){
        ...
        String messageToSend = "Hello! , [" + this.authorName + "]("
                +  this.authorURL + ")"
                + " has closed the merge request: [" +this.mergeRequestTitle
                + "](" + this.viewLink + ") "
    ...
    private MessageBody constructMessageBodyInHtml(){
        String titile = "Merge request Closed";
        String messageToSend = "Hello! , " + HtmlContentCreator.getAnchor(this.authorURL, this.authorName)
                + " has closed the merge request: " + HtmlContentCreator.getAnchor(this.viewLink, this.mergeRequestTitle)
