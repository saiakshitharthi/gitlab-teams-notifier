...
public class MergeRequestApprovedPayloadParser extends MergeRequestPayloadParser {


    public MergeRequestApprovedPayloadParser(GitLabAPI gitLabAPI) {
        super(gitLabAPI);
    }
    @Override
    ...
    public List<Message> getMessagesToBeSentInHtml() {
        ...
        messageBody.setMessageTypeID(MessageTypeID.MERGE_REQUEST_APPROVED);
        Set<String> allReceivers = getAllReceivers();
    ...
    private MessageBody constructMessageBody() {
        ...
        String messageToSend = "Hello! , [" + this.authorName + "]("
                + this.authorURL + ")"
                + " has approved the merge request: [" + this.mergeRequestTitle
                + "](" + this.viewLink + ") "
    ...
    private MessageBody constructMessageBodyInHtml(){
        String title = "Merge request Approved";
        String messageToSend = "Hello! , " + HtmlContentCreator.getAnchor(this.authorURL, this.authorName)
                + " has approved the merge request: " + HtmlContentCreator.getAnchor(this.viewLink, this.mergeRequestTitle)
