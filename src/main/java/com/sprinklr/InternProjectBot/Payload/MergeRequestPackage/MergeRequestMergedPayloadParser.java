...
public class MergeRequestMergedPayloadParser extends MergeRequestPayloadParser{

    public MergeRequestMergedPayloadParser(GitLabAPI gitLabAPI){
        super(gitLabAPI);
    }
    ...
    public List<Message> getMessagesToBeSent() {
        ...
        messageBody.setMessageTypeID(MessageTypeID.MERGE_REQUEST_MERGED);
    ...
    private MessageBody constructMessageBody(){
        ...
        String messageToSend = "Hello! , [" + this.authorName + "]("
                +  this.authorURL + ")"
                + " has merged the merge request: [" +this.mergeRequestTitle
                + "](" + this.viewLink + ") "
    ...
    private MessageBody constructMessageBodyInHtml(){
        String title = "Merge request merged";
        String messageToSend = "Hello! , "+ HtmlContentCreator.getAnchor(this.authorURL, this.authorName)
                + " has merged the merge request: "+ HtmlContentCreator.getAnchor(this.viewLink, this.mergeRequestTitle)
                + " from branch : "+ HtmlContentCreator.bold(this.sourceBranch) + " to branch : "+ HtmlContentCreator.bold(this.targetBranch)
